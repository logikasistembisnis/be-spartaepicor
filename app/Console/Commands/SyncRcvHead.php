...
use App\Http\Controllers\RcvHeadController;
use Illuminate\Support\Facades\Log;
use Carbon\Carbon;

class SyncRcvHead extends Command
{
    /**
     * Nama perintah (command)
     * @var string
     */
    protected $signature = 'sync:epicor-rcvhead {arg1? : Periode Awal (ym) atau StartDate (Ymd)} 
     ...
     * @var string
     */
    protected $description = 'Mengambil data RcvHead dari Epicor API dan menyinkronkannya (UPSERT).';
    ...
    public function handle()
    {
        ...
        if ($this->option('all')) {
            ...
            $this->info('Memulai sinkronisasi SEMUA data RcvHead dari Epicor');

            // Panggil controller dengan null, null
            $result = $controller->syncRcvHeadData(null, null);
        ...
        if ($endPeriod) {
            ...
            while ($current <= $end) {
                ...
                $result = $controller->syncRcvHeadData($periodString, $startDate);
