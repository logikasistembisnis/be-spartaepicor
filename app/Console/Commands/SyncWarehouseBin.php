...
class SyncWarehouseBin extends Command
{
    /**
     * Nama perintah (command)
     * @var string
     */
    protected $signature = 'sync:epicor-warehousebin {arg1? : Periode Awal (ym) atau StartDate (Ymd)} 
     ...
     * @var string
     */
    protected $description = 'Mengambil data WarehouseBin harian dari Epicor API dan menyinkronkannya (UPSERT).';
    ...
    public function handle()
    {
        ...
        if ($this->option('all')) {
            ...
            $this->info('Memulai sinkronisasi SEMUA data WarehouseBin dari Epicor');

            // Panggil controller dengan null, null
            $result = $controller->syncWarehouseBinData(null, null);
        ...
        if ($endPeriod) {
            ...
            while ($current <= $end) {
                ...
                $result = $controller->syncWarehouseBinData($periodString, $startDate);
        ...
        // --- SCENARIO 1: TUNGGAL (atau DEFAULT) ---
            ...
            if ($hasArgs) {
                 $this->info('Memulai sinkronisasi data WarehouseBin dari Epicor');
            } else {
            ...
            // Panggil controller
            $result = $controller->syncWarehouseBinData($period, $startDate); 
