...
use App\Http\Controllers\UD10Controller;
use Illuminate\Support\Facades\Log;
use Carbon\Carbon;

class SyncUD10 extends Command
{
    /**
     * Nama perintah (command)
     * @var string
     */
    protected $signature = 'sync:epicor-ud10 {arg1? : Periode Awal (ym) atau StartDate (Ymd)} 
     ...
     * @var string
     */
    protected $description = 'Mengambil data UD10 dari Epicor API dan menyinkronkannya (UPSERT).';
    ...
    public function handle()
    {
        ...
        if ($this->option('all')) {
            ...
            $this->info('Memulai sinkronisasi SEMUA data UD10 dari Epicor');

            // Panggil controller dengan null, null
            $result = $controller->syncUD10Data(null, null);
        ...
        if ($endPeriod) {
            ...
            while ($current <= $end) {
                ...
                $result = $controller->syncUD10Data($periodString, $startDate);
        ...
        // --- SCENARIO 1: TUNGGAL (atau DEFAULT) ---
            ...
            if ($hasArgs) {
                 $this->info('Memulai sinkronisasi data UD10 dari Epicor');
            } else {
            ...
            // Panggil controller
            $result = $controller->syncUD10Data($period, $startDate); 
