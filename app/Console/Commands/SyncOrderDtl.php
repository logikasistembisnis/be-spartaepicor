...
class SyncOrderDtl extends Command
{
    /**
     * Nama perintah (command)
     * @var string
     */
    protected $signature = 'sync:epicor-orderdtl {arg1? : Periode Awal (ym) atau StartDate (Ymd)} 
     ...
     * @var string
     */
    protected $description = 'Mengambil data OrderDtl dari Epicor API dan menyinkronkannya (UPSERT).';
    ...
    {
        $controller = new OrderDtlController();
        ...
        if ($this->option('all')) {
            ...
            $this->info('Memulai sinkronisasi SEMUA data OrderDtl dari Epicor');

            // Panggil controller dengan null, null
            $result = $controller->syncOrderDtlData(null, null);
        ...
        if ($endPeriod) {
            ...
            while ($current <= $end) {
                ...
                $result = $controller->syncOrderDtlData($periodString, $startDate);
        ...
        // --- SCENARIO 1: TUNGGAL (atau DEFAULT) ---
            ...
            if ($hasArgs) {
                 $this->info('Memulai sinkronisasi data OrderDtl dari Epicor');
            } else {
            ...
            // Panggil controller
            $result = $controller->syncOrderDtlData($period, $startDate); 
