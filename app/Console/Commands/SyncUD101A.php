...
class SyncUD101A extends Command
{
     ...
     * @var string
     */
    protected $signature = 'sync:epicor-ud101a {arg1? : Periode Awal (ym) atau StartDate (Ymd)} 
    ...
    public function handle()
    {
        ...
        if ($this->option('all')) {
            ...
            $this->info('Memulai sinkronisasi SEMUA data UD101A dari Epicor');

            // Panggil controller dengan null, null
            $result = $controller->syncUD101AData(null, null);
        ...
        if ($endPeriod) {
            ...
            while ($current <= $end) {
                ...
                $result = $controller->syncUD101AData($periodString, $startDate);
        ...
        // --- SCENARIO 1: TUNGGAL (atau DEFAULT) ---
            ...
            // Panggil controller
            $result = $controller->syncUD101AData($period, $startDate); 
