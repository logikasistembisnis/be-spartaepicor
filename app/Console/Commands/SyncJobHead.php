...
use App\Http\Controllers\JobHeadController;
use Illuminate\Support\Facades\Log;
use Carbon\Carbon;

class SyncJobHead extends Command
{
    /**
     * Nama perintah (command)
     * @var string
     */
    protected $signature = 'sync:epicor-jobhead {arg1? : Periode Awal (ym) atau StartDate (Ymd)} 
     ...
     * @var string
     */
    protected $description = 'Mengambil data JobHead dari Epicor API dan menyinkronkannya (UPSERT).';
    ...
    {
        $controller = new JobHeadController();
        ...
        if ($this->option('all')) {
            ...
            $this->info('Memulai sinkronisasi SEMUA data JobHead dari Epicor');

            // Panggil controller dengan null, null
            $result = $controller->syncJobHeadData(null, null);
        ...
        if ($endPeriod) {
            ...
            while ($current <= $end) {
                ...
                $result = $controller->syncJobHeadData($periodString, $startDate);
        ...
        // --- SCENARIO 1: TUNGGAL (atau DEFAULT) ---
            ...
            if ($hasArgs) {
                 $this->info('Memulai sinkronisasi data JobHead dari Epicor');
            } else {
            ...
            // Panggil controller
            $result = $controller->syncJobHeadData($period, $startDate); 
