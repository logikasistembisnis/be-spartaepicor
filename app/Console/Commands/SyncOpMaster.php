...
class SyncOpMaster extends Command
{
     ...
     * @var string
     */
    protected $signature = 'sync:epicor-opmaster {arg1? : Periode Awal (ym) atau StartDate (Ymd)} 
    ...
    public function handle()
    {
        ...
        if ($this->option('all')) {
            ...
            $this->info('Memulai sinkronisasi SEMUA data OpMaster dari Epicor');

            // Panggil controller dengan null, null
            $result = $controller->syncOpMasterData(null, null);
        ...
        if ($endPeriod) {
            ...
            while ($current <= $end) {
                ...
                $result = $controller->syncOpMasterData($periodString, $startDate);
        ...
        // --- SCENARIO 1: TUNGGAL (atau DEFAULT) ---
            ...
            // Panggil controller
            $result = $controller->syncOpMasterData($period, $startDate); 
