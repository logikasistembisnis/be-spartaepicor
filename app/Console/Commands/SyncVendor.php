...
class SyncVendor extends Command
{
     ...
     * @var string
     */
    protected $signature = 'sync:epicor-vendor {arg1? : Periode Awal (ym) atau StartDate (Ymd)} 
     ...
     * @var string
     */
    protected $description = 'Mengambil data Vendor harian dari Epicor API dan menyinkronkannya (UPSERT).';
    ...
    public function handle()
    {
        ...
        if ($this->option('all')) {
            ...
            $this->info('Memulai sinkronisasi SEMUA data Vendor dari Epicor');

            // Panggil controller dengan null, null
            $result = $controller->syncVendorData(null, null);
        ...
        if ($endPeriod) {
            ...
            while ($current <= $end) {
                ...
                $result = $controller->syncVendorData($periodString, $startDate);
        ...
        // --- SCENARIO 1: TUNGGAL (atau DEFAULT) ---
            ...
            if ($hasArgs) {
                 $this->info('Memulai sinkronisasi data Vendor dari Epicor');
            } else {
            ...
            // Panggil controller
            $result = $controller->syncVendorData($period, $startDate); 
