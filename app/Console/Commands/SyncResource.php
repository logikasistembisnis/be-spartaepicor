...
class SyncResource extends Command
{
     ...
     * @var string
     */
    protected $signature = 'sync:epicor-resource {arg1? : Periode Awal (ym) atau StartDate (Ymd)} 
     ...
     * @var string
     */
    protected $description = 'Mengambil data Resource harian dari Epicor API dan menyinkronkannya (UPSERT).';
    ...
    public function handle()
    {
        ...
        if ($this->option('all')) {
            ...
            $this->info('Memulai sinkronisasi SEMUA data Resource dari Epicor');

            // Panggil controller dengan null, null
            $result = $controller->syncResourceData(null, null);
        ...
        if ($endPeriod) {
            ...
            while ($current <= $end) {
                ...
                $result = $controller->syncResourceData($periodString, $startDate);
        ...
        // --- SCENARIO 1: TUNGGAL (atau DEFAULT) ---
            ...
            if ($hasArgs) {
                 $this->info('Memulai sinkronisasi data Resource dari Epicor');
            } else {
            ...
            // Panggil controller
            $result = $controller->syncResourceData($period, $startDate); 
