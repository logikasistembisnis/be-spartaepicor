...
use App\Http\Controllers\UD101Controller;
use Illuminate\Support\Facades\Log;
use Carbon\Carbon;

class SyncUD101 extends Command
{
    /**
     * Nama perintah (command)
     * @var string
     */
    protected $signature = 'sync:epicor-ud101 {arg1? : Periode Awal (ym) atau StartDate (Ymd)} 
     ...
     * @var string
     */
    protected $description = 'Mengambil data UD101 harian dari Epicor API dan menyinkronkannya (UPSERT).';
    ...
    {
        $controller = new UD101Controller();
        ...
        if ($this->option('all')) {
            ...
            $this->info('Memulai sinkronisasi SEMUA data UD101 dari Epicor');

            // Panggil controller dengan null, null
            $result = $controller->syncUD101Data(null, null);
        ...
        if ($endPeriod) {
            ...
            while ($current <= $end) {
                ...
                $result = $controller->syncUD101Data($periodString, $startDate);
        ...
        // --- SCENARIO 1: TUNGGAL (atau DEFAULT) ---
            ...
            if ($hasArgs) {
                 $this->info('Memulai sinkronisasi data UD101 dari Epicor');
            } else {
            ...
            // Panggil controller
            $result = $controller->syncUD101Data($period, $startDate); 
