...
class SyncPoDetail extends Command
{
     ...
     * @var string
     */
    protected $signature = 'sync:epicor-podetail {arg1? : Periode Awal (ym) atau StartDate (Ymd)} 
    ...
    public function handle()
    {
        ...
        if ($this->option('all')) {
            ...
            $this->info('Memulai sinkronisasi SEMUA data PoDetail dari Epicor');

            // Panggil controller dengan null, null
            $result = $controller->syncPoDetailData(null, null);
        ...
        if ($endPeriod) {
            ...
            while ($current <= $end) {
                ...
                $result = $controller->syncPoDetailData($periodString, $startDate);
