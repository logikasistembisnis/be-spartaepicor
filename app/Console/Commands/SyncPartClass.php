...
class SyncPartClass extends Command
{
    /**
     * Nama perintah (command)
     * @var string
     */
    protected $signature = 'sync:epicor-partclass {arg1? : Periode Awal (ym) atau StartDate (Ymd)} 
     ...
     * @var string
     */
    protected $description = 'Mengambil data PartClass harian dari Epicor API dan menyinkronkannya (UPSERT).';
    ...
    {
        $controller = new PartClassController();
        ...
        if ($this->option('all')) {
            ...
            $this->info('Memulai sinkronisasi SEMUA data PartClass dari Epicor');

            // Panggil controller dengan null, null
            $result = $controller->syncPartClassData(null, null);
        ...
        if ($endPeriod) {
            ...
            while ($current <= $end) {
                ...
                $result = $controller->syncPartClassData($periodString, $startDate);
        ...
        // --- SCENARIO 1: TUNGGAL (atau DEFAULT) ---
            ...
            if ($hasArgs) {
                 $this->info('Memulai sinkronisasi data PartClass dari Epicor');
            } else {
            ...
            // Panggil controller
            $result = $controller->syncPartClassData($period, $startDate); 
