...
class SyncWarehouse extends Command
{
     ...
     * @var string
     */
    protected $signature = 'sync:epicor-warehouse {arg1? : Periode Awal (ym) atau StartDate (Ymd)} 
     ...
     * @var string
     */
    protected $description = 'Mengambil data Warehouse harian dari Epicor API dan menyinkronkannya (UPSERT).';
    ...
    public function handle()
    {
        ...
        if ($this->option('all')) {
            ...
            $this->info('Memulai sinkronisasi SEMUA data Warehouse dari Epicor');

            // Panggil controller dengan null, null
            $result = $controller->syncWarehouseData(null, null);
        ...
        if ($endPeriod) {
            ...
            while ($current <= $end) {
                ...
                $result = $controller->syncWarehouseData($periodString, $startDate);
        ...
        // --- SCENARIO 1: TUNGGAL (atau DEFAULT) ---
            ...
            if ($hasArgs) {
                 $this->info('Memulai sinkronisasi data Warehouse dari Epicor');
            } else {
            ...
            // Panggil controller
            $result = $controller->syncWarehouseData($period, $startDate); 
