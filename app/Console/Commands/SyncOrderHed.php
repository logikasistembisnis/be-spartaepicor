...
use App\Http\Controllers\OrderHedController;
use Illuminate\Support\Facades\Log;
use Carbon\Carbon;

class SyncOrderHed extends Command
{
    /**
     * Nama perintah (command)
     * @var string
     */
    protected $signature = 'sync:epicor-orderhed {arg1? : Periode Awal (ym) atau StartDate (Ymd)} 
     ...
     * @var string
     */
    protected $description = 'Mengambil data OrderHed dari Epicor API dan menyinkronkannya (UPSERT).';
    ...
    {
        $controller = new OrderHedController();
        ...
        if ($this->option('all')) {
            ...
            $this->info('Memulai sinkronisasi SEMUA data OrderHed dari Epicor');

            // Panggil controller dengan null, null
            $result = $controller->syncOrderHedData(null, null);
        ...
        if ($endPeriod) {
            ...
            while ($current <= $end) {
                ...
                $result = $controller->syncOrderHedData($periodString, $startDate);
        ...
        // --- SCENARIO 1: TUNGGAL (atau DEFAULT) ---
            ...
            if ($hasArgs) {
                 $this->info('Memulai sinkronisasi data OrderHed dari Epicor');
            } else {
            ...
            // Panggil controller
            $result = $controller->syncOrderHedData($period, $startDate); 
