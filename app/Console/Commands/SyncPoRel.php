...
use App\Http\Controllers\PoRelController;
use Illuminate\Support\Facades\Log;
use Carbon\Carbon;

class SyncPoRel extends Command
{
    /**
     * Nama perintah (command)
     * @var string
     */
    protected $signature = 'sync:epicor-porel {arg1? : Periode Awal (ym) atau StartDate (Ymd)} 
     ...
     * @var string
     */
    protected $description = 'Mengambil data PoRel harian dari Epicor API dan menyinkronkannya (UPSERT).';
    ...
    {
        $controller = new PoRelController();
        ...
        if ($this->option('all')) {
            ...
            $this->info('Memulai sinkronisasi SEMUA data PoRel dari Epicor');

            // Panggil controller dengan null, null
            $result = $controller->syncPoRelData(null, null);
        ...
        if ($endPeriod) {
            ...
            while ($current <= $end) {
                ...
                $result = $controller->syncPoRelData($periodString, $startDate);
        ...
        // --- SCENARIO 1: TUNGGAL (atau DEFAULT) ---
            ...
            if ($hasArgs) {
                 $this->info('Memulai sinkronisasi data PoRel dari Epicor');
            } else {
            ...
            // Panggil controller
            $result = $controller->syncPoRelData($period, $startDate); 
