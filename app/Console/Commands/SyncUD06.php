...
use App\Http\Controllers\UD06Controller;
use Illuminate\Support\Facades\Log;
use Carbon\Carbon;

class SyncUD06 extends Command
{
    /**
     * Nama perintah (command)
     * @var string
     */
    protected $signature = 'sync:epicor-ud06 {arg1? : Periode Awal (ym) atau StartDate (Ymd)} 
     ...
     * @var string
     */
    protected $description = 'Mengambil data UD06 harian dari Epicor API dan menyinkronkannya (UPSERT).';
    ...
    public function handle()
    {
        ...
        if ($this->option('all')) {
            ...
            $this->info('Memulai sinkronisasi SEMUA data UD06 dari Epicor');

            // Panggil controller dengan null, null
            $result = $controller->syncUD06Data(null, null);
        ...
        if ($endPeriod) {
            ...
            while ($current <= $end) {
                ...
                $result = $controller->syncUD06Data($periodString, $startDate);
        ...
        // --- SCENARIO 1: TUNGGAL (atau DEFAULT) ---
            ...
            if ($hasArgs) {
                 $this->info('Memulai sinkronisasi data UD06 dari Epicor');
            } else {
            ...
            // Panggil controller
            $result = $controller->syncUD06Data($period, $startDate); 
