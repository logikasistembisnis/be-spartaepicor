...
class SyncCustomer extends Command
{
     ...
     * @var string
     */
    protected $signature = 'sync:epicor-customer {arg1? : Periode Awal (ym) atau StartDate (Ymd)} 
    ...
    public function handle()
    {
        ...
        if ($this->option('all')) {
            ...
            $this->info('Memulai sinkronisasi SEMUA data Customer dari Epicor');

            // Panggil controller dengan null, null
            $result = $controller->syncCustomerData(null, null);
        ...
        if ($endPeriod) {
            ...
            while ($current <= $end) {
                ...
                $result = $controller->syncCustomerData($periodString, $startDate);
        ...
        // --- SCENARIO 1: TUNGGAL (atau DEFAULT) ---
            ...
            // Panggil controller
            $result = $controller->syncCustomerData($period, $startDate); 
