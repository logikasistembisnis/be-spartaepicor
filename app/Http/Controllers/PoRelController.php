...
class PoRelController extends Controller
{
     ...
     */
    public function syncPoRelData(?string $period = null, ?string $startDate = null): array
    {
        ...
        do {
            ...
            ->timeout(600)
            ->get(env('EPICOR_API_URL'). '/ETL_PoRel/Data', $apiParams);
            ...
            foreach ($dataChunks as $chunk) {
                $chunk = collect($chunk)
                ->reverse()
                ->unique(fn($r) => $r['PORel_PONum'] . '-' . $r['PORel_POLine'] . '-' . $r['PORel_PORelNum'])
                ->values()
                ->toArray();
